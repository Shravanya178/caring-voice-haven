...
import { Dices, RefreshCw, Chess, Target, X, Circle, Calculator } from 'lucide-react';
...
const GamesCollection: React.FC = () => {
  ...
  // ============= HANGMAN STATE =============
  const [hangmanState, setHangmanState] = useState<HangmanState>({
    word: '',
    guessedLetters: [],
    remainingAttempts: 6,
    status: 'playing',
  ...
  const generateQuestions = () => {
    ...
    for (let i = 0; i < 10; i++) {
      const operation = Math.floor(Math.random() * 3); // 0: addition, 1: subtraction, 2: multiplication
  ...
  const handleLetterGuess = (letter: string) => {
    ...
    // Update remaining attempts if incorrect guess
  ...
  useEffect(() => {
    // Initialize the memory game when component first loads
    initializeMemoryGame();
  }, []);

  return (
    <div className="space-y-6 animate-fade-in mt-4 md:ml-16">
      ...
      <Tabs value={activeGame} onValueChange={setActiveGame} className="w-full">
        <TabsList className="grid grid-cols-4 w-full">
          ...
          <TabsTrigger value="hangman" className="flex items-center gap-2">
            <Chess className="h-4 w-4" /> Hangman
          </TabsTrigger>
        </TabsList>
